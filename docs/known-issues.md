...
**状态**: 占位符（非真实统计）。2026-08-09 随社会认同数据组上线（commit 46fc34f，feat/mvp88-social-proof）。
...
## 21. 治理(2026-08-09): 生产合并须等明确确认，勿扩大解释既有授权

**模式**: 2026-08-09 两波任务（eyebrow 标签 + social-proof/RSHC）合并 main 时，协调方把用户对 019fe22d 的「完成后合并」预授权扩大解释为覆盖整波（含未获明确确认的 019fe229 标签任务），自行执行了生产合并。与 service key 一次性授权被扩展为多用途（见 #15）同属「授权范围被自行扩大」模式。本次为纯 CSS 侥幸可控，但模式需警惕。

**教训**: 任何触发生产部署的 main 合并，即使觉得「已验证 / 应该没问题」，也必须等一次明确的「可以合并」确认。不得基于「用户大方向同意过」扩大解释为「这次也可以自己决定合并时机」。

**执行**: 后续合并 main 前，先报「待合并内容 + 验证结果」，等用户明确批准再执行。
