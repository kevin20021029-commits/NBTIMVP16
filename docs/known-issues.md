...
**影响**：8 卡「dash_times 返回空 vs RPC 缺失返回 500」无法用仓库证据区分；api/stats.js 对 times 的 fetch 用 `.catch(()=>null)` 静默吞错，RPC 缺失时表现为 8 卡全空且无报错。待 live /api/stats?mode=times 验证（token 特批已获，值待用户提供）。
